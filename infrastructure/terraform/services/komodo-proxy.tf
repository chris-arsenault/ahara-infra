...
resource "aws_iam_role_policy" "komodo_proxy_ssm" {
  name = "${local.prefix}-komodo-proxy-ssm"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        "arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/ahara/*"
      ]
    }]
  })
}
...
resource "aws_lambda_function" "komodo_proxy" {
  function_name = "${local.prefix}-komodo-proxy"

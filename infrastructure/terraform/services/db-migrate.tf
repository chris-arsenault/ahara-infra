...
locals {
  # Prepend the shared prefix project (its db_name == local.prefix)
  all_migration_projects = merge(var.migration_projects, { (local.prefix) = { db_name = local.prefix } })
}

# --- S3 bucket for migration files ---

resource "aws_s3_bucket" "migrations" {
  bucket = "${local.prefix}-migrations-${data.aws_caller_identity.current.account_id}"
}
...
resource "aws_iam_role_policy" "db_migrate_s3_ssm" {
  name = "${local.prefix}-db-migrate-s3-ssm"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = ["arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/ahara/db/*"]
      }
    ]
  })
}
...
resource "aws_lambda_function" "db_migrate" {
  function_name = "${local.prefix}-db-migrate"
  ...
  environment {
    variables = {
      ...
      DB_USER           = aws_db_instance.ahara.username

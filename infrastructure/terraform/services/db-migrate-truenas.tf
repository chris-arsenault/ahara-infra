...
resource "aws_iam_role_policy" "db_migrate_truenas_ssm" {
  name = "${local.prefix}-db-migrate-truenas-ssm"
    ...
    Statement = [
      {
        ...
        Resource = ["arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/ahara/truenas/*"]
      },
      {
        Effect   = "Allow"
        Action   = ["ssm:PutParameter", "ssm:GetParameter"]
        Resource = ["arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/ahara/truenas-db/*"]
      }
    ]
  })
}
...
resource "aws_lambda_function" "db_migrate_truenas" {
  function_name = "${local.prefix}-db-migrate-truenas"
